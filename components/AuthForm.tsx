    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        try {
            if(type==="sign-up"){
                ...
                const userCrendials=await createUserWithEmailAndPassword(auth, email, password);

                const result = await signUp({
                    uid: userCrendials.user.uid,
                    name: name!,
                    email,
                    password,
                })
                ...
                const userCrendials=await signInWithEmailAndPassword(auth, email, password);

                const idToken=await userCrendials.user.getIdToken();
        ...
        <div className={"card-border lg:min-w-[566px]"}>
            <div className="flex flex-col gap-6 card py-14 px-10">
                <div className="flex flex-row gap-2 justify-center">
                    <Image src="/logo.svg" width={38} height={32} alt="PrepWise Logo"/>
            ...
            <Form {...form}>
                ...
                <p className="text-center">
                    ...
                    <Link className={"font-bold ml-2 text-user-primary"} href={!isSignIn?'/sign-in':'/sign-up'}>
